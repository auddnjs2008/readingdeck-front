...
type CommunityPostGraphViewProps = {
  snapshot: CommunityPostDetail["snapshot"];
};

export function CommunityPostGraphView({ snapshot }: CommunityPostGraphViewProps) {
  ...
  const graphPreview = useMemo(
    () => buildGraphPreview(snapshot.nodes, snapshot.connections),
    [snapshot.connections, snapshot.nodes]
  );

  const resolvedSelectedNodeId =
    selectedNodeId ?? cardNodes[0]?.id ?? snapshot.nodes[0]?.id ?? null;
  ...
  }, [snapshot.nodes, resolvedSelectedNodeId]);
  ...
  }, [snapshot.connections, resolvedSelectedNodeId]);
...
function SelectedCardNode({
  ...
  selectedNode: CommunityPostDetail["snapshot"]["nodes"][number];
}) {
