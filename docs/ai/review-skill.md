...
description: ReadingDeck의 PR diff를 리뷰할 때 사용한다. 타입/린트 우회, React 경계, 상태 원본, 모듈 구조, 테스트/모킹 경계를 점검한다.
---
...
PR diff를 ReadingDeck의 현재 구조와 규칙으로 리뷰한다. 버그 가능성, 경계 위반, 검증 누락을 찾는다.
...
- 명령과 설치된 도구는 현재 `package.json`을 기준으로 확인한다. 존재하지 않는 검사 명령이나 레이어를 요구하지 않는다.
- 현재 내부 경로 import를 사용하므로 barrel Public API가 없다는 이유만으로 finding을 만들지 않는다. 실제 순환 의존이나 책임 위반을 근거로 판단한다.
- 개인 책·덱·카드·프로필 데이터는 CSR과 React Query로 조회한다. 공개 커뮤니티는 SSR을 사용하고 피드는 hydration을 사용한다.
- 서버 데이터의 편집 초안은 로컬 state에 둘 수 있다. 읽기용 복제와 구분하고 저장·취소·갱신 충돌을 확인한다.
- 문서 적용을 이유로 요청 범위 밖의 구조 개편이나 테스트 도구 도입을 제안하지 않는다.
