...
아래 도구별 규칙은 해당 도구가 설치된 경우에 적용한다. 스킬 적용만을 이유로 테스트 의존성을 추가하지 않는다.

ReadingDeck의 현재 검증 명령은 `pnpm lint`, `pnpm typecheck`, `pnpm build`다. 기존 회귀 테스트는 다음 Node 내장 러너 명령으로 실행한다.
...
MSW, Vitest, Nuqs, Playwright는 현재 프로젝트 의존성이 아니다. 기존 캐시·인터셉터 계약 테스트의 API mock과 Axios adapter는 해당 계약 범위에서 허용하되 실제 HTTP나 사용자 흐름 검증으로 설명하지 않는다. 테스트에서 실제 사용자 데이터에 쓰기 요청을 보내지 않고, 모킹 여부와 검증하지 못한 범위를 밝힌다.
...
- 책 추가·수정·삭제가 서재 통계와 홈 요약에 이어지는 흐름

사용자 흐름 통합 테스트는 HTTP 경계에서 모킹하는 것을 우선한다. MSW가 설치돼 있다면 활용한다. 기존 캐시·인터셉터 계약 테스트의 API mock이나 Axios adapter는 그 검증 범위 안에서 유지할 수 있다.
...
- `책을 삭제하면 서재 통계와 빈 상태가 함께 갱신된다`
- `products scenario를 error로 바꾼 뒤 reset하면 success 응답으로 복구된다`
- `mock API가 지연 응답해도 document 진입을 막지 않고 서재 목록으로 전환된다`
...
- URL 상태 adapter (해당 라이브러리를 사용하는 경우)
...
현재는 서버와 브라우저 모두 `NEXT_PUBLIC_API_BASE_URL`을 사용한다. 테스트 시 양쪽이 같은 mock API를 바라보도록 설정한다. 이 값은 build 시점에 브라우저 번들에 들어가므로 production E2E build에도 같은 값을 넣는다.
...
테스트에서 가장 중요한 판단은 “무엇을 assert할지”와 “어디까지 mock할지”다. 이 두 가지를 AI가 제안했더라도 그대로 받아쓰지 않고, 요청된 요구사항과 코드 구조에 맞는지 직접 확인한 뒤 확정한다.
