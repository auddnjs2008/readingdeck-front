...
ReadingDeck의 개인 책·덱·카드·프로필 데이터는 CSR과 React Query로 조회한다. 공개 커뮤니티는 SSR을 사용하고 피드는 hydration을 사용한다. 기존 내부 경로 import와 entities/features/widgets 구조를 우선하며 새 레이어나 barrel Public API를 강제하지 않는다.
...
- 서재 목록 API 요청 조립은 endpoint/query string 세부사항이므로 컴포넌트 밖으로 분리한다.
- URL query parser는 서재 목록 URL 상태 정책에 묶여 있어 Hook 내부에 둔다.
