...
- 사용자 흐름 통합 테스트는 가능한 한 HTTP 경계에서 모킹한다. MSW는 도입된 경우 사용하며 필수 설치하지 않는다.
- 기존 캐시·인터셉터 계약 테스트의 API mock이나 Axios adapter는 그 계약 범위 안에서 허용한다. 이를 실제 HTTP 검증으로 설명하지 않는다.
